...
@ManagedBean(name = "TaskCreationNewTaskViewDefaultviewView")
@SessionScoped
@ExpireWithPageFlow
@DTManagedBean(displayName = "TaskCreationNewTaskView/default", beanType = BeanType.PAGE)
public class TaskCreationNewTaskViewDefaultviewView extends com.webmethods.caf.faces.bean.task.BaseTaskDetailsPageBean {
	...
	private static final String[][] TASKDISPLAYPROVIDER_PROPERTY_BINDINGS = new String[][] {
		{"#{TaskDisplayProvider.taskID}", "#{TaskCreationNewTaskView.taskID}"},
	};
	...
	public String cancelView() {
		try {
			...
			String url = getTaskCreationNewTaskView().getFinishUrl();
	...
	public String completeTask() {
		try {
			if( !getTaskCreationNewTask().isUpdateable() ){
			...
			// do the work
			getTaskCreationNewTask().completeTask(); 
	...
	public String submitTask() {
		try {
			...
			// do the work
			getTaskCreationNewTask().applyChanges();
		} catch (Exception e) {
			error(e);
			log(e);
		}
	...
	public String assignToPrincipals() {
		try {
			...
			TaskContentProvider tp = getTaskCreationNewTask();
	...
	private static final String[][] INITIALIZE_PROPERTY_BINDINGS = new String[][] {
		{"#{TaskCreationNewTask.reset}", null}
	};
	private transient caf.war.TEST21.taskcreationnewtaskview.TaskCreationNewTaskView taskCreationNewTaskView = null;
	...
	private static final String[][] TASKCREATIONNEWTASK_PROPERTY_BINDINGS = new String[][] {
		{"#{TaskCreationNewTask.taskID}", "#{TaskCreationNewTaskView.taskID}"},
		{"#{TaskCreationNewTask.autoAccept}", "false"},
		{"#{TaskCreationNewTask.adhocRouting}", "false"},
	};
	...
	protected void beforeRenderResponse() {
		...
		try {
			getTaskCreationNewTask().refresh();
		} catch( Exception ex ) {
			error(ex);
			log(ex);
		}
	...
	public caf.war.TEST21.taskcreationnewtaskview.TaskCreationNewTaskView getTaskCreationNewTaskView()  {
		if (taskCreationNewTaskView == null) {
		    taskCreationNewTaskView = (caf.war.TEST21.taskcreationnewtaskview.TaskCreationNewTaskView)resolveExpression("#{TaskCreationNewTaskView}");
		}
		return taskCreationNewTaskView;
	}
